...
+++++++++++++++++++++++
Compile the host program with OpenCL offload using the following command: ``g++  -O3 -std=c++11 matmul_ocl.cpp  -lOpenCL -locl_util -o matmpy``. ``libTIOpenCL.so`` is TI's OpenCL Runtime library. ``libocl_util.so`` provides utility functions such as ``ocl_code_error``.
